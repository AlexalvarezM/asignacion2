  ...
  const cargarProductos = (inputValue) => {
    return new Promise((resolve) => {
      const filtrados = productos
        ...
        .map(p => ({
          value: p.id_producto,
          label: `${p.nombre_producto} - C$ ${parseFloat(p.precio_producto || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} / $ ${parseFloat(p.precio_dolar || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
          data: p
        }));
      resolve(filtrados);
    });
          ...
          <Col lg={7} md={6}>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Select value={moneda} onChange={(e) => setMoneda(e.target.value)}>
                <option value="cordoba">Córdobas (C$)</option>
              ...
              <Card.Body className="p-0" style={{ maxHeight: "400px", overflowY: "auto" }}>
                {detalles.length === 0 ? (
                  ...
                  <ListGroup variant="flush">
                    {detalles.map((det) => {
                      const precioMostrar = moneda === "dolar" ? (det.precio_dolar || 0) : det.precio;
                      const subtotalMostrar = det.cantidad * precioMostrar;
                      const simbolo = moneda === "dolar" ? "$" : "C$";
                      return (
                        <ListGroup.Item key={det.id_producto} className="d-flex justify-content-between align-items-center">
                          <div>
                            ...
                            <small className="text-muted">
                              {det.cantidad} × {simbolo} {precioMostrar.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </small>
                          </div>
                          <div className="text-end">
                            <div className="fw-bold">{simbolo} {subtotalMostrar.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                            <Button
              ...
              <Card.Footer className="bg-light">
                <div className="d-flex justify-content-between align-items-center fs-4 fw-bold">
                  ...
                  <span className="text-success">
                    {moneda === "dolar" ? "$" : "C$"} {totalGeneral.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
              </Card.Footer>
            </Card>
